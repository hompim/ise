            ...
            <div class="grid sm:mb-10 sm:gap-x-20 sm:gap-y-8 gap-y-5 gap-x-5 md:grid-cols-10">
                ...
                <div class="md:hidden col-span-5">
                    @if ($is_edit)
                        @if (Auth::user()->userable->academy->profile_verif_status != 'Terverifikasi' &&
                            Auth::user()->userable->academy->profile_verif_status != 'Tahap Verifikasi')
                ...
                <div class="col-span-10 md:col-span-2 md:block hidden">
                    ...
                    @else
                        @if (Auth::user()->userable->academy->profile_verif_status != 'Terverifikasi' &&
                            Auth::user()->userable->academy->profile_verif_status != 'Tahap Verifikasi')
                            <button type="button" class="text-blue-500 justify-self-end" wire:click="toEditMode()">
                            </button>
                        @endif
                    @endif
                </div>
                ...
                <div id="informasi_tim" class="col-span-10 md:col-span-6">
                    ...
                    <div>
                        <label for="institute_name" class="mb-2 font-bold text-gray-400 mt-4">Asal
                            {{ Auth::user()->userable->academy_type == 'academy_junior' ? 'Sekolah' : 'Perguruan Tinggi' }}</label><br>
                        @if ($is_edit)
                        ...
                        <div>
                            <div class="" x-data="{ isUploading: false, progress: 0 }"
                                ...
                                <div class="flex flex-col items-center justify-center md:p-3 w-full">
                                    <label for="kartu_pelajar" class="capitalize text-gray-400">KTM/Surat Keterangan
                                    ...
                                    @if (Auth::user()->userable->academy->profile_verif_status != 'Terverifikasi' &&
                                        Auth::user()->userable->academy->profile_verif_status != 'Tahap Verifikasi' &&
                                        $is_edit)
                                        <button type="button" onclick="$('#photo1').click()"
                                        ...
                                        <input type="file" wire:model.defer="photo1" class="form-control-file"
                                            name="photo1" id="photo1" accept=".jpg,.jpeg,.png" hidden>
                                    @endif
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="" x-data="{ isUploading: false, progress: 0 }"
                                ...
                                <div class="flex flex-col items-center justify-center md:p-3 w-full">
                                    ...
                                    @if (Auth::user()->userable->academy->profile_verif_status != 'Terverifikasi' &&
                                        Auth::user()->userable->academy->profile_verif_status != 'Tahap Verifikasi' &&
                                        $is_edit)
                                        <button type="button" onclick="$('#photo2').click()"
                                            class="bg-red-400 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-2">
                                            <i class="fas fa-cloud-upload-alt mr-2"></i>Unggah File
                                        </button>
                                        <input type="file" wire:model.defer="photo2" class="form-control-file"
                                            name="photo2" id="photo2" accept=".jpg,.jpeg,.png" hidden>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                @if (Auth::user()->userable->academy->member2_id)
                    ...
                    <div id="anggota_3" class="col-span-10 md:col-span-6">
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                <div class="">
                                    <label for="member_3_name" class="mb-2 font-bold text-gray-400">Nama Lengkap
                                        Anggota
                                        3</label><br>
                                    @if ($is_edit)
                                        <input wire:model.defer="member_3_name" type="text" id="member_3_name"
                                            name="member_3_name" class="registration-form input-text"
                                ...
                                <div class="">
                                    ...
                                    @if ($is_edit)
                                        <input wire:model.defer="member_3_email" type="email" id="member_3_email"
                                            name="member_3_email" class="registration-form input-text"
                                            style="color: black;margin-top:0;" required>
                                    @else
                                ...
                                <div class="">
                                    <label for="member_3_whatsapp" class="mb-2 font-bold text-gray-400 mt-4">Nomor WA
                                        ...
                                        3</label><br>
                                    @if ($is_edit)
                                        <input wire:model.defer="member_3_whatsapp" type="text"
                                            id="member_3_whatsapp" name="member_3_whatsapp"
                                            class="registration-form input-text" style="color: black;margin-top:0;"
                                            required onkeydown="checkWhatsapp(this.value,'phone_3_error')"
                                ...
                                <div>
                                    <label for="member_3_twibbon" class="mb-2 font-bold text-gray-400 mt-4">Link Post
                                        ...
                                        3</label><br>
                                    @if ($is_edit)
                                        <input wire:model.defer="member_3_twibbon" type="text"
                                            id="member_3_twibbon" name="member_3_twibbon"
                                            class="{{ $readonly ? 'bg-gray-100' : '' }} registration-form input-text"
                                            style="color: black;margin-top:0;" required {{ $readonly ? 'disabled' : '' }}>
                                    @else
                            ...
                            <div>
                                <div class="" x-data="{ isUploading: false, progress: 0 }"
                                    ...
                                    x-on:livewire-upload-progress="progress = $event.detail.progress">
                                    <div class="flex flex-col items-center justify-center md:p-3 w-full">
                                        <label for="kartu_pelajar" class="capitalize text-gray-400">KTM/Surat
                                        ...
                                        <img src="{{ $photo3 ? (is_string($photo3) ? asset('storage/' . $photo3) : $photo3->temporaryUrl()) : asset('/img/global/placeholder-image.png') }}"
                                            class="object-fit mx-auto" alt="Kartu Pelajar" id="card_preview"
                                            style="max-height:50vh">
                                        @if (Auth::user()->userable->academy->profile_verif_status != 'Terverifikasi' &&
                                            Auth::user()->userable->academy->profile_verif_status != 'Tahap Verifikasi' &&
                                            $is_edit)
                                            <button type="button" onclick="$('#photo3').click()"
                                                class="bg-red-400 hover:bg-red-700 text-white font-bold py-2 px-4 rounded mt-2">
                                                <i class="fas fa-cloud-upload-alt mr-2"></i>Unggah File
                                            </button>
                                            <input type="file" wire:model.defer="photo3" class="form-control-file"
                                                name="photo3" id="photo3" accept=".jpg,.jpeg,.png" hidden>
                                        @endif
                                    </div>
                                </div>
                            </div>
