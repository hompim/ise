    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
            <div class="flex items-center h-10">
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm ">
                        <div class="relative flex-grow mb-3 focus-within:z-10">
                            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                <svg class="w-5 h-5 text-white" viewBox="0 0 20 20" stroke="currentColor" fill="none">
            ...
            @if($this->activeFilters)
                <span class="text-xl text-white uppercase">@lang('Filter active')</span>
            @endif
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if(count($this->massActionsOptions))
                    <div class="flex items-center justify-center space-x-1">
                        ...
                        <select wire:model="massActionOption" class="px-3 text-xs font-medium tracking-wider uppercase bg-black border border-green-400 space-x-2 rounded-md leading-4 focus:outline-none" id="datatables_mass_actions">
                        ...
                        <button
                            ...
                            class="flex items-center px-4 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-black border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none" type="submit" title="Submit"
                        >Go</button>
                    </div>
                @endif
                ...
                @foreach ($columnGroups as $name => $group)
                    <button wire:click="toggleGroup('{{ $name }}')"
                            class="px-3 py-2 text-xs font-medium tracking-wider text-green-500 uppercase bg-black border border-green-400 rounded-md leading-4 hover:bg-green-200 focus:outline-none">
        ...
        <div wire:loading.class="opacity-50" class="rounded-lg @unless($complex || $this->hidePagination) rounded-b-none @endunless shadow-lg bg-black max-w-screen overflow-x-scroll border-2 @if($this->activeFilters) border-blue-500 @else border-transparent @endif @if($complex) rounded-b-none border-b-0 @endif">
            ...
            @if($this->results->isEmpty())
                <p class="p-3 text-lg text-center text-white">
        ...
        @unless($this->hidePagination)
            <div class="max-w-screen bg-black @unless($complex) rounded-b-lg @endunless border-4 border-t-0 border-b-0 @if($this->activeFilters) border-blue-500 @else border-transparent @endif">
